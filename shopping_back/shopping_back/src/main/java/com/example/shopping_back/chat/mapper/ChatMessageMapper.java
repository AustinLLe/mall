...
public interface ChatMessageMapper {
    ...
    @Insert("INSERT INTO chat_message (cov_id, sender_id, content, price_value, create_time, is_read) " +
            "VALUES (#{covId}, #{senderId}, #{content}, #{priceValue}, #{createTime}, #{isRead})")
    @Options(useGeneratedKeys = true, keyProperty = "cmId", keyColumn = "cm_id")
    int insert(ChatMessage msg);

    @Select("SELECT cm_id, cov_id, sender_id, content, price_value, create_time, is_read " +
    ...
    @Update("UPDATE chat_message SET cov_id=#{covId}, sender_id=#{senderId}, content=#{content}, " +
            "price_value=#{priceValue}, create_time=#{createTime}, is_read=#{isRead} WHERE cm_id=#{cmId}")
